...
switch idx
	case 0 % Circle with cubic basis functions
		par = struct('par', @(t) repmat([0;0], 1, length(t)) + [0.5*cos(2*pi*t); 0.5*sin(2*pi*t)], 'gradnorm', @(t) pi*ones(size(t)), ...
            ...
            'ppw', 6, 'xi', 0.04);
		return
	case 1 % Circle with linear basis functions and point source at (1,1)
		par = struct('par', @(t) repmat([0;0], 1, length(t)) + [0.5*cos(2*pi*t); 0.5*sin(2*pi*t)], 'gradnorm', @(t) pi*ones(size(t)), ...
            ...
            'bc', @(k,x)  -1*besselh(0,1, k*sqrt( (x(1,:)' - 1).^2 + (x(2,:)' -1).^2) ), 'dbf', 1, 'ppw', 6, 'xi', 0.04);
		return
	case 2 % Ellipse with superposition of plane waves
		par = struct('par', @(t) repmat([0;0], 1, length(t)) + [0.3*cos(2*pi*t); 0.5*sin(2*pi*t)], 'gradnorm', ...
			@(t) 2*pi*sqrt(0.3^2+0.5^2)*ones(size(t)), 'corners', [], 'bc', ...
            @(k,x) -1*exp(1i*k*(x')*[cos(-pi/3); sin(-pi/3)]) ...
            -2*exp(1i*k*(x')*[cos(pi/3); sin(pi/3)]) -3*exp(1i*k*(x')*[cos(pi); sin(pi)]), ...
            'dbf', 1, 'ppw', 6, 'xi', 0.04);
		return
	case 3 % Near-inclusion with one plane wave
		x = [0.7454 0.6947 0.5933 0.3836 0.2108 0.1440 0.2546 0.4597 0.5472 0.5219 0.3491 0.1694 0.2131 0.4366 0.6417 0.7200;...
            ...
            bc = @(k,x) -1*exp(1i*k*(x')*[cos(0); sin(0)]);
	case 4 % Near-convex with point source at (1,1)
		x = [ 0.6071  0.6141  0.6302  0.6233  0.5611  0.4965  0.4343  0.3906  0.3836  0.3975  0.4067  0.3952  0.3813  ...
            ...
            bc = @(k,x) -1*besselh(0,1, k*sqrt( (x(1,:)' - 1).^2 + (x(2,:)' -1).^2) );
	case 5 % Two circles with point source at (1,1)
		par = struct('obsts', [struct('par', @(t) repmat([-0.5;-0.5], 1, length(t)) + [0.5*cos(2*pi*t); 0.5*sin(2*pi*t)], 'gradnorm',... 
			...
			@(t) pi*ones(size(t)), 'normal', @(t) [cos(2*pi*t); sin(2*pi*t)], 'corners', [], 'dbf', 1, 'ppw', 10)], ...
        	    'bc', @(k,x) -1*besselh(0,1, k*sqrt( (x(1,:)' - 1).^2 + (x(2,:)' -1).^2) ), 'xi', 3e-3);
		return
	case 6 % Three ellipses with one incident plane wave
		par = struct('obsts', [struct('par', @(t) repmat([0;0], 1, length(t)) + [0.3*cos(2*pi*t); 0.5*sin(2*pi*t)], 'gradnorm',...
            ...
            'xi', 3e-3);
		return
	case 7 % Near-inclusion and circle with superposition of plane waves
	        tmp = getObst(3);
        	tmp = rmfield(tmp,'bc');
	        tmp = rmfield(tmp,'xi');
            ...
            par = struct('obsts', [tmp struct('par', @(t) repmat([-0.5;-0.5], 1, length(t)) + [0.5*cos(2*pi*t); 0.5*sin(2*pi*t)], ...
                ...
                'bc', @(k,x) -1*exp(1i*k*(x')*[cos(-pi/3); sin(-pi/3)]) ...
                -2*exp(1i*k*(x')*[cos(pi/3); sin(pi/3)]) -3*exp(1i*k*(x')*[cos(pi); sin(pi)]), 'xi', 1e-3);
		return
	case 8 % Polygon with superposition of plane waves
        	y = [1 -1 0 -1  1; ...
	             1 1  0 -1 -1];
	        dbf = 1; ppw = 9; xi = 0.04;
            bc = @(k,x) -1*exp(1i*k*(x')*[cos(-pi/3); sin(-pi/3)]) ...
                -2*exp(1i*k*(x')*[cos(pi/3); sin(pi/3)]) -3*exp(1i*k*(x')*[cos(pi); sin(pi)]);
	case 9 % One self-reflection with one incident plane wave
		x = [1 -1 0 -1  1; ...
	         1 1  0 -1 -1];
	        dbf = 1; ppw = 15; xi = 0.04;
            bc = @(k,x) -1*exp(1i*k*(x')*[cos(0); sin(0)]);
	case 10 % One self-reflection line pieces with superposition of plane waves
		par = struct('par', @(t) onePar, 'gradnorm', @(t) oneGrad(t), 'corners', [], 'bc', ...
            @(k,x) -1*exp(1i*k*(x')*[cos(-pi/3); sin(-pi/3)]) ...
            -2*exp(1i*k*(x')*[cos(pi/3); sin(pi/3)]) -3*exp(1i*k*(x')*[cos(pi); sin(pi)]), ...
            'dbf', 1, 'ppw', 15, 'xi', 0.04);
		return
    case 11 % Two ellipses with point source at (1,1)
...
par.bc = bc; par.dbf = dbf; par.ppw = ppw; par.xi = xi;
